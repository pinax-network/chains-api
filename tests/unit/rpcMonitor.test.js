...
import { getMonitoringResults, getMonitoringStatus, startRpcHealthCheck, startMonitoring, getClientsByChain } from '../../rpcMonitor.js';
...
describe('RPC Monitor', () => {
  ...
  describe('Parsed client field', () => {
    it('populates result.client for a recognized client version', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
      ...
      vi.mocked(jsonRpcCall)
        .mockResolvedValueOnce('Geth/v1.14.5-stable-xxx/linux-amd64/go1.22.5')
        .mockResolvedValueOnce('0x12345');
      ...
      const results = getMonitoringResults();
      const ethResults = results.results.filter(r => r.chainId === 1);
      expect(ethResults[0].client).toMatchObject({
        name: 'geth',
    ...
    it('populates result.client with known=false for an unknown client', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});

      vi.mocked(getAllEndpoints).mockReturnValue([
        { chainId: 999, name: 'Mystery Chain', rpc: ['https://mystery.rpc.com'] }
      ]);

      vi.mocked(jsonRpcCall)
        .mockResolvedValueOnce('some-new-client/v0.1.0')
        .mockResolvedValueOnce('0x1');

      await startMonitoring();

      const results = getMonitoringResults();
      const mysteryResults = results.results.filter(r => r.chainId === 999);
      expect(mysteryResults[0].client).toMatchObject({
    ...
    it('aggregates clients across working endpoints for a chain', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
      ...
      // rpc1: geth v1.14.5, rpc2: geth v1.14.5 (same version), rpc3: erigon v2.60.0
      vi.mocked(jsonRpcCall)
        .mockResolvedValueOnce('Geth/v1.14.5/linux/go1.22')
        .mockResolvedValueOnce('0x1')
        .mockResolvedValueOnce('Geth/v1.14.5/linux/go1.22')
        .mockResolvedValueOnce('0x1')
        .mockResolvedValueOnce('erigon/v2.60.0/linux/go1.22')
        .mockResolvedValueOnce('0x1');
      ...
      // Sorted by nodeCount descending — geth should be first with 2 nodes
      ...
      expect(summary.clients[1]).toMatchObject({
        name: 'erigon',
        nodeCount: 1
      });
    ...
    it('returns null from getClientsByChain when chain has no monitoring data', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    it('returns an array across all chains when chainId is omitted', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    it('counts endpoints with no parsed client as unknownNodes', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});

      vi.mocked(getAllEndpoints).mockReturnValue([
        { chainId: 1, name: 'Ethereum', rpc: ['https://a.rpc.com', 'https://b.rpc.com'] }
      ]);

      // a: client version unavailable, b: known geth
      vi.mocked(jsonRpcCall)
        .mockRejectedValueOnce(new Error('not supported'))
        .mockResolvedValueOnce('0x1')
        .mockResolvedValueOnce('Geth/v1.14.5')
        .mockResolvedValueOnce('0x1');
    ...
    it('sets result.client to null when web3_clientVersion is unavailable', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
      ...
      const ethResults = results.results.filter(r => r.chainId === 1);
      expect(ethResults[0].clientVersion).toBe('unavailable');
      expect(ethResults[0].client).toBeNull();
